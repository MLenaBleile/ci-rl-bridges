...
p_combined <- ggplot(combined_data, aes(x = trial)) +
  ...
  annotate("text", x = 10, y = 10, label = "epsilon", parse = TRUE,
           color = "black", hjust = 0) +
  ...
  scale_y_continuous(
    ...
    sec.axis = sec_axis(~ . / 10,
                        name = expression(epsilon ~ "(Exploration Probability)"))
  ) +
...
ggsave("../figures/action_value_updates_combined.png", p_combined, 
