...
n <- 1500
group1 <- runif(n, 2.5,5.5)  # Changed mean from 5 to 4
group2 <- runif(n, 6.5,9.5)  # Changed mean from 7 to 8
...
cutoff1 <- quantile(group1, 0.7)
cutoff2 <- quantile(group2, 0.7)
...
lines(d1$x, d1$y * scale_factor, col = "darkgreen", lwd = 2)
lines(d2$x, d2$y * scale_factor, col = "chocolate", lwd = 2)
...
polygon(c(d2$x[d2$x >= cutoff2], cutoff2), 
        ...
        col = "burlywood2", border = NA)


# Add vertical lines for means
abline(v = true_mean1, col = "darkgreen", lwd = 2, lty = 2)
abline(v = true_mean2, col = "chocolate", lwd = 2, lty = 2)
abline(v = trunc_mean1, col = "darkgreen", lwd = 2, lty = 3)
abline(v = trunc_mean2, col = "chocolate", lwd = 2, lty = 3)

# Add text labels for means with better spacing
text(true_mean1 - 0.2, 0.45, expression(theta[1]), col = "darkgreen", cex = 1.2, pos = 3)
text(true_mean2 + 0.2, 0.45, expression(theta[2]), col = "chocolate", cex = 1.2, pos = 3)
text(trunc_mean1 - 0.2, 0.35, expression(hat(theta)[1](x[0])), col = "darkgreen", cex = 1.2, pos = 1)
text(trunc_mean2 + 0.2, 0.35, expression(hat(theta)[2](x[0])), col = "chocolate", cex = 1.2, pos = 1)

# Add arrows and labels for differences with improved positioning
arrows(true_mean1 + 0.2, 0.5, true_mean2 - 0.2, 0.5, code = 3, length = 0.1, lwd = 2)
text((true_mean1 + true_mean2)/2, 0.52, 
     paste("True difference =", round(true_diff, 1)), cex = 1.1)

arrows(trunc_mean1 + 0.2, 0.42, trunc_mean2 - 0.2, 0.42, code = 3, length = 0.1, lwd = 2)
text((trunc_mean1 + trunc_mean2)/2, 0.39, 
     paste("Estimated difference\n=", round(trunc_diff, 1)), cex = 1.1)

# Add "missing" labels with better positioning
text(cutoff1 + 0.8, 0.15, "missing", col = "darkgreen", cex = 1.1)
text(cutoff2 + 0.8, 0.15, "missing", col = "chocolate", cex = 1.1)

# Add legend with more space and shorter text
legend("topleft", 
       legend = c("Group 1", "Group 2", "True Means", "Est. Means"),
       col = c("darkgreen", "chocolate", "black", "black"),
       lty = c(1, 1, 2, 3),
       lwd = c(2, 2, 2, 2),
       cex = 0.9,
